...
3. `20260522100000_fix_rls_recursion_companies_tf_requests.sql` — 기업 조회 RLS 재귀 수정(CTEs 사용).
4. `20260607000001_add_populate_request_matches_rpc.sql` — `populate_request_matches(request_id uuid)` RPC 함수 추가(service_role 실행, AI 매칭 결과를 `request_matches` 테이블에 채움).
5. `20260607000002_add_contracts_storage_bucket.sql` — `contracts` Storage 버킷 생성(비공개, PDF 전용, 10 MB 크기 제한), `contracts_insert_service_role`·`contracts_select_authenticated` 정책 추가.
6. `20260607000004_add_profiles_insert_policy.sql` — `profiles_insert_own` INSERT 정책 추가(`id = auth.uid()`), 트리거 미적용 환경에서 가입 action의 방어적 upsert에 필요.

**QA 시드 (`emanitri@gmail.com`)** 는 마이그레이션에서 제외했습니다. `supabase/seeds/qa/emanitri.sql` 을 SQL Editor에서 수동 실행하세요.

**정리**: `db reset` 시 위 6파일이 순서대로 실행된 뒤 `seed.sql`이 실행됩니다.
...
npx supabase db reset --version 20260522100000 --yes
npx supabase db reset --version 20260607000001 --yes
npx supabase db reset --version 20260607000002 --yes
npx supabase db reset --version 20260607000004 --yes
```
